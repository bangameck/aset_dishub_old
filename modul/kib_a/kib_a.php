...
<div class="app-content content">
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Kartu Invertaris Barang A (Tanah)</h4>
                                </div>
                                    ...
                                    <div class="card-body card-dashboard">
                                        <div class="table-responsive">
                                            <table class="table table-striped dataex-html5-selectors">
                                                ...
                                                <tbody>
                                                <?php 
                                                        $kib = $db->query("SELECT *, YEAR(tgl_perolehan) as y, MONTH(tgl_perolehan) as m, DAY(tgl_perolehan) as d FROM barang a, kib_a b WHERE a.id_brg_ack=b.id_brg_ack ORDER BY id_brg ASC");
                                                        $no = 0;
                                                        while($k=$kib->fetch_assoc()) :
                                                            $no++;
                                                            if(empty($k['ket_kib_a'])){
                                                                $ket = '-';
                                                            } else {
                                                                $ket = $k['ket_kib_a'];
                                                            }
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><ul class="list-unstyled users-list m-0  d-flex align-items-center">
                                                                ...
                                                                <li data-toggle="tooltip" data-popup="tooltip-custom" data-placement="bottom" data-original-title="Detail Aset" class="avatar pull-up">
                                                                    <a href="<?= base_url(); ?>kib_a/det/<?= $k['id_brg_ack']; ?>"><span class="avatar-content"><i class="avatar-icon feather icon-chevrons-right"></i></span></a>
                                                                </li>
                                                            </ul>
                                                        </td>
                                                    </tr>
    ...
    <?php case 'detail' : 
         $det = $db->query("SELECT *, YEAR(tgl_perolehan) as y, MONTH(tgl_perolehan) as m, DAY(tgl_perolehan) as d,
                                    YEAR(tgl_sertifikat) as ys, MONTH(tgl_sertifikat) as ms, DAY(tgl_sertifikat) as ds, 
                                    YEAR(tgl_penghapusan) as yp, MONTH(tgl_penghapusan) as mp, DAY(tgl_penghapusan) as dp FROM barang a, kib_a b, users c WHERE a.id_brg_ack=b.id_brg_ack AND a.usr_input=c.id_usr AND a.id_brg_ack='$_GET[id]'")->fetch_assoc();
         $tgl_update = $db->query("SELECT YEAR(updated_at) as yu, MONTH(updated_at) as mu, DAY(updated_at) as du FROM kib_a WHERE id_brg_ack='$_GET[id]'")->fetch_assoc();
         $tgl_create = $db->query("SELECT YEAR(created_at) as yc, MONTH(created_at) as mc, DAY(created_at) as dc FROM kib_a WHERE id_brg_ack='$_GET[id]'")->fetch_assoc();
        $nama_update = $db->query("SELECT * FROM users a, barang b WHERE a.id_usr=b.usr_update AND b.id_brg_ack='$_GET[id]'")->fetch_assoc();
        if ($det['tgl_sertifikat'] == '0000-00-00'){
            $tgl = '-';
        } else {
            $tgl = tgl_indo(date($det['ys'].'-'.$det['ms'].'-'.$det['ds']));
        }
        ?>
                            ...
                            <div class="row mb-5 mt-2">
                                ...
                                <h5><?= $det['id_brg']; ?> - <?= $det['k_brg']; ?> - <?= $det['p_kib_a']; ?>
                                    ...
                                    <table class="table table-bordered mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td> <?= luas($det['luas_a']); ?></td>
                                            </tr>
                                            <tr>
                                                <th scope="row">Nomor Sertifikat</th>
                                                <td> <?= $det['n_sertifikat']; ?></td>
                                            </tr>
                                            <tr>
                                    ...
                                    <!-- <p>
                                        ...
                                        <b>Tanggal Perolehan :</b> <?= $tgl; ?></span> <br>
                                        <b>Alamat : </b> <?= $det['alamat_a']; ?></span> <br>
                                        <b>Luas :</b> <?= luas($det['luas_a']); ?></span> <br>
                                        <b>Nomor Sertifikat :</b> <?= $det['n_sertifikat']; ?></span> <br>
                                        <b>Tanggal Setifikat :</b> <?= $tgl; ?></span> <br>
                                        <b>Penggunaan :</b> <?= $det['p_kib_a']; ?></span> <br>
                                        <b>Keterangan :</b> <?= $det['ket_kib_a']; ?></span>.</p>
                                        <p> Di Input Oleh : <b><?= $det['nama']; ?></b><br>
                                        Pada Tanggal :  <?= tgl_indo(date($tgl_create['yc'].'-'.$tgl_create['mc'].'-'.$tgl_create['dc'])); ?>
                                    </p> -->
                                    <hr>
