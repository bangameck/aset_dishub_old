...
<div class="app-content content">
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Kartu Invertaris Barang D (Jalan Irigasi dan Jaringan)</h4>
                                </div>
                                    ...
                                    <div class="card-body card-dashboard">
                                        <div class="table-responsive">
                                            <table class="table table-striped dataex-html5-selectors">
                                                ...
                                                <tbody>
                                                <?php 
                                                        $kib = $db->query("SELECT *, YEAR(tgl_perolehan) as y, MONTH(tgl_perolehan) as m, DAY(tgl_perolehan) as d FROM barang a, kib_d b WHERE a.id_brg_ack=b.id_brg_ack ORDER BY id_brg ASC");
                                                        $no = 0;
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><ul class="list-unstyled users-list m-0  d-flex align-items-center">
                                                                ...
                                                                <li data-toggle="tooltip" data-popup="tooltip-custom" data-placement="bottom" data-original-title="Detail Aset" class="avatar pull-up">
                                                                    <a href="<?= base_url(); ?>kib_d/det/<?= $k['id_brg_ack']; ?>"><span class="avatar-content"><i class="avatar-icon feather icon-chevrons-right"></i></span></a>
                                                                </li>
                                                            </ul>
                                                        </td>
                                                    </tr>
    ...
    <?php case 'detail' : 
         $det = $db->query("SELECT *, YEAR(tgl_perolehan) as y, MONTH(tgl_perolehan) as m, DAY(tgl_perolehan) as d,
                                    YEAR(tgl_imb_d) as yi, MONTH(tgl_imb_d) as mi, DAY(tgl_imb_d) as di, 
                                    YEAR(tgl_penghapusan) as yp, MONTH(tgl_penghapusan) as mp, DAY(tgl_penghapusan) as dp FROM barang a, kib_d b, users c WHERE a.id_brg_ack=b.id_brg_ack AND a.usr_input=c.id_usr AND a.id_brg_ack='$_GET[id]'")->fetch_assoc();
         $tgl_update = $db->query("SELECT YEAR(updated_at) as yu, MONTH(updated_at) as mu, DAY(updated_at) as du FROM kib_d WHERE id_brg_ack='$_GET[id]'")->fetch_assoc();
         $tgl_create = $db->query("SELECT YEAR(created_at) as yc, MONTH(created_at) as mc, DAY(created_at) as dc FROM kib_d WHERE id_brg_ack='$_GET[id]'")->fetch_assoc();
